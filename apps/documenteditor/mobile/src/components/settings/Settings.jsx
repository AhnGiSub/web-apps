...
import DocumentInfoController from "../settings/controller/DocumentInfo";
import {
  DocumentFormats,
  DocumentMargins,
} from "../settings/document-settings/DocumentSettings";
...
const SettingsList = withTranslation()((props) => {
  ...
  const navbar = (
    <Navbar title={_t.textSettings}>
      {!props.inPopover && (
        <NavRight>
          <Link popupClose=".settings-popup">{_t.textDone}</Link>
        </NavRight>
      )}
    </Navbar>
  );
  ...
  const onoptionclick = (page) => {
    if (props.onOptionClick) props.onOptionClick(page);
  };
      ...
      <Page>
        ...
        <List>
          ...
          <ListItem
            ...
            link="#"
            onClick={onoptionclick.bind(this, "/document-info/")}
          >
  ...
  render() {
    ...
    return show_popover ? (
      <Popover
        ...
        onPopoverClosed={() => this.props.onclosed()}
      >
        <SettingsList
          inPopover={true}
      ...
      <Popup
        ...
        onPopupClosed={() => this.props.onclosed()}
      >
...
const Settings = (props) => {
  useEffect(() => {
    if (Device.phone) f7.popup.open(".settings-popup");
